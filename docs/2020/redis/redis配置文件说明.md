...
# 3.2里的参数，是否开启保护模式，默认开启。要是配置里没有指定bind和密码。开启该参数后，redis只会本地进行访问，
...
# 绑定本机地址（本机网卡对应的IP地址，每一个网卡都有一个ip地址）；注意：并不是绑定来自其它计算机的IP地址，否则将无法启动redis
# 如果你的bind设置为：bind 0.0.0.0，表示所有主机都可以连接到redis，前提：你的服务器必须开放redis的端口，这时需要设置密码，多一层保护
bind 127.0.0.1
